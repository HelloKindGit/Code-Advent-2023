...
def extrapolate_next_value(history):
    ...
    return extrapolate_values(sequences)

def calculate_histories():
    file_path = "day_9/part1/input.txt"  # 'day_9/part1/test.txt'
    with open(file_path, 'r') as file:
        data = [line.strip() for line in file]

    sum_of_extrapolated_values = sum(extrapolate_next_value(list(map(int, line.split()))) for line in data)
